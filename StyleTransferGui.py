...
class MainWindow(wx.Frame):
    ...
    def __binds(self):
        """绑定函数;"""
        self.Bind(wx.EVT_TIMER, self.next_frame, self.timer)
        self.img_out_win.Bind(wx.EVT_ERASE_BACKGROUND, self.on_erase_background)

        self.Bind(wx.EVT_MENU, self.on_exit, self.menuExit)
        self.Bind(wx.EVT_MENU, self.set_hyper, self.menuhyper)
        self.Bind(wx.EVT_MENU, self.on_helpdoc, self.menuhelpdoc)
...
class TransThread(threading.Thread):
    ...
    def run(self):
        ...
        self.win.log_msg("train time:" + str(dt))
        self.win.log_msg("total time:" + str(dt2))
